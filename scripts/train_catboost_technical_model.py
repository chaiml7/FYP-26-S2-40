"""
Train a binary CatBoost technical-analysis direction model and save it locally.

Run from repo root after syncing indicators:
    python scripts/train_catboost_technical_model.py --all
    python scripts/train_catboost_technical_model.py --symbol NVDA
    python scripts/train_catboost_technical_model.py --symbol NVDA --threshold 0.005
    python scripts/train_catboost_technical_model.py --all --train-before-date 2026-06-04
"""
import argparse
import os
import sys
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train/evaluate a binary CatBoost technical model. Target is "
    ...
    parser.add_argument(
        "--max-features",
        type=int,
        default=DEFAULT_CATBOOST_MAX_FEATURES,
        help="Top CatBoost feature-importance features to keep. Default: 45",
    )
    parser.add_argument(
        "--output-path",
        default=str(CATBOOST_ARTIFACT_PATH),
        help="Where to save the CatBoost .joblib artifact.",
    )
...
def print_result(result: dict[str, Any]) -> None:
    ...
    print(f"categorical_features: {result.get('categorical_features', [])}")
