...
def test_tune_financial_model_route(mock_tune):
    mock_tune.return_value = {
        ...
        "candidate_count": 20,
        "best_result": {
            "candidate_name": "regularized_depth_2",
            "latest_holdout_metrics": {"accuracy": 0.5},
        },
        "saved_model": None,
    }
...
@patch("backend.routes.financial_routes.tune_binary_financial_model")
def test_tune_binary_financial_model_route(mock_tune):
    mock_tune.return_value = {
        "source_dataset_rows": 3784,
        "candidate_count": 50,
        "best_result": {
    ...
    response = client.post(
        "/api/financial/model/tune-binary",
        json={"top_n": 3},
    )

    assert response.status_code == 200
    assert response.json()["candidate_count"] == 50
    mock_tune.assert_called_once_with(top_n=3)


@patch("backend.routes.financial_routes.train_binary_financial_model")
def test_train_binary_financial_model_route(mock_train):
    mock_train.return_value = {
        "saved_model": {
    ...
    response = client.post(
        "/api/financial/model/train-binary",
        json={"top_n": 3},
    )
...
@patch("backend.routes.financial_routes.generate_binary_financial_prediction")
def test_generate_binary_financial_prediction_route(mock_generate):
    mock_generate.return_value = {
        "ticker": "AAPL",
        "prediction": "neutral",
    ...
    response = client.post(
        "/api/financial/predictions-binary/aapl"
...
@patch("backend.routes.financial_routes.read_latest_binary_financial_prediction")
def test_latest_binary_prediction_route(mock_latest):
    ...
    response = client.get("/api/financial/predictions-binary/AAPL/latest")

    assert response.status_code == 200
    assert response.json()["model_type"] == "binary_financial"


@patch("backend.routes.financial_routes.read_latest_binary_financial_prediction", return_value=None)
def test_latest_binary_prediction_returns_404_when_missing(mock_latest):
    response = client.get("/api/financial/predictions-binary/AAPL/latest")
